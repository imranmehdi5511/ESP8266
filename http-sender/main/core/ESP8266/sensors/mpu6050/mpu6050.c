...
char* get_mpu6050(int a)
{
    ...
    {	
    	case 0:	/* Aceler X */
    	sprintf(buffer, "%s",str_float( ((slave_read_byte(ACCEL_XOUT_H) << 8) | slave_read_byte(ACCEL_XOUT_H + 1) ) / 1638.4 ) );
    	s=buffer;
        return s ; 
        case 1:	/* Aceler Y */
    	sprintf(buffer, "%s",str_float( ((slave_read_byte(ACCEL_YOUT_H) << 8) | slave_read_byte(ACCEL_YOUT_H + 1) ) / 1638.4 ) );
    	s=buffer;
        return s ; 
        case 2:
		sprintf(buffer, "%s",str_float( ((slave_read_byte(ACCEL_ZOUT_H) << 8) | slave_read_byte(ACCEL_ZOUT_H + 1) ) / 1638.4 ) );
    	s=buffer;
        return s ; /* Aceler Z */
        case 3:
    	sprintf(buffer, "%s",str_float( ((slave_read_byte(GYRO_XOUT_H) << 8) | slave_read_byte(GYRO_XOUT_H + 1) ) / 131.0 ) );
    	s=buffer;
        return s ; /* Gyrosc X */
        case 4:
    	sprintf(buffer, "%s",str_float( ((slave_read_byte(GYRO_YOUT_H) << 8) | slave_read_byte(GYRO_YOUT_H + 1) ) / 131.0 ) );
    	s=buffer;
        return s ; /* Gyrosc Y */
        case 5:
    	sprintf(buffer, "%s",str_float( ((slave_read_byte(GYRO_ZOUT_H) << 8) | slave_read_byte(GYRO_ZOUT_H + 1) ) / 131.0 ) );
    	s=buffer;
        return s ; /* Gyrosc Z */
    }
    return "0";
